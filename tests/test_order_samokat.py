...
class TestOrderSamokat:
    ...
    def test_order_header_button_without_comment_success(self, driver):
        main_page = MainPageScooter(driver)
        order_page = OrderPage(driver)
        main_page.click_header_order_button()
        order_page.wait_for_load_header_for_whom_scooter()
        order_page.set_order_conditions_for_whom_scooter(data.random_name(),data.random_surname(),data.random_address(),data.random_telephone())
